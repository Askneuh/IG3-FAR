...
char* trouverSalonDuClient(struct client c) {
    for (int i = 0; i < nb_salons; i++) {
        for (ClientNode* cur = salons[i].clients; cur; cur = cur->next) {
            if (cur->data.adClient.sin_addr.s_addr == c.adClient.sin_addr.s_addr &&
                cur->data.adClient.sin_port == c.adClient.sin_port)
            {
                return salons[i].nom;
            }
        }
...
/**
 * Retourne 1 si 'pseudo' est déjà listé dans le salon 'nom';
 * 0 sinon.
 * Compare uniquement les username stockés dans la liste chaînée.
 */
int pseudoDansSalon(const char *nom, const char *pseudo) {
    int idx = salonExiste(nom);
    if (idx < 0) return 0;
    for (ClientNode *cur = salons[idx].clients; cur; cur = cur->next) {
        if (strcmp(cur->data.username, pseudo) == 0)
            return 1;
    }
    return 0;
}
...
// /**
//  * Parcourt tous les salons, et si le client 'c' (avec c.username)
//  * y figurait déjà (testé par pseudoDansSalon), on l’ajoute “pour de vrai”
//  * en mémoire (avec son adClient correct).
//  */
// void restaurerClientDansSesSalons(struct client c) {
//     for (int i = 0; i < nb_salons; i++) {
//         const char *nomSalon = salons[i].nom;
//         if (pseudoDansSalon(nomSalon, c.username)) {
//             // Vérifie qu'il n'y a pas déjà une entrée avec cette adresse
//             // (selon ta logique addClient() / tests existants)
//             retirerClientDuSalon(nomSalon, c);
//             ajouterClientAuSalon(nomSalon, c);
//             printf("🔄 %s automatiquement ajouté au salon \"%s\" lors de la connexion.\n",
//                    c.username, nomSalon);
//         }
//     }
// }
